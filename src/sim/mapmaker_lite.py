...
hist_v = histo[v_selec, :] / looplen
hist_v = hist_v.T / np.nanmax(hist_v)

map_v  = map_coadd[v_idx, :]
map_v  = map_v.T / np.nanmax(map_v)

hist_h = histo[:, h_selec] / looplen
hist_h = hist_h / np.nanmax(hist_h)

map_h  = map_coadd[:, h_idx]
map_h  = map_h / np.nanmax(map_h)
